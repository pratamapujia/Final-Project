...
if(!isset($_SESSION['login'])){
    ...
    //membuat id otomatis
    $query = "SELECT max(id_anamnesis) as id_anamnesis From anamnesis";
    $hasil = mysqli_query($conn,$query);
    $data = mysqli_fetch_array($hasil);
    $id_anamnesis = $data['id_anamnesis'];
    $auto_id = (int) substr($id_anamnesis, 3, 5);
    $auto_id++;
    $char = "ANM";
    $id_anamnesis = $char.sprintf("%03s",$auto_id);

    //proses submit
    if(isset($_POST['submit'])){
        if( tambah_anamnesis($_POST) > 0){
            echo "<script>
                setTimeout(function(){
                    Swal.fire({
                        icon: 'success',
                            ...
                            <input type="hidden" name="id_anamnesis" value="<?php echo $id_anamnesis?>" >
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" type="text" name="riwayat_penyakit" required>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label>Jumlah Keguguran <span class="text-danger">*</span></label>
                                        <input class="form-control" type="text" name="jml_keguguran" required>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label>Cara Persalinan <span class="text-danger">*</span></label>
                                        <input class="form-control" type="text" name="cara_persalinan" required>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label>Kehamilan Ke <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control" name="kehamilan" required>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label>Tekanan Darah <span class="text-danger">*</span></label>
                                        <input class="form-control" type="text" name="tekanan_darah" required>
                                    </div>
                                </div>
                            </div>
